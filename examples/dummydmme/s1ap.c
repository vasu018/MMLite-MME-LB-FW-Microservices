...
s1ap_mme_handle_initial_ue_message(struct rte_mbuf* pkt, uint8_t slice_id) 
{
	...
	if(get_msg_type(s1ap_packet) == DETACH_REQUEST) {
...
#ifdef LITE_SERVICE
			nas_send_init_context_setup_req(get_enb_ue_s1ap_id(s1ap_packet));
			free(s1ap_packet);
			return 0;
#endif
		} else {
		...
		if(kvs_get_64(IMSI_HTABLE,(get_imsi(s1ap_packet))) != NULL) {
			kvs_del_64(IMSI_HTABLE, (get_imsi(s1ap_packet)));
			kvs_del(ENB_HTABLE, get_enb_ue_s1ap_id(s1ap_packet));
			rte_free(ue_context);
		}
#else
		if(kvs_get_64(IMSI_HTABLE,(get_imsi(s1ap_packet))) == NULL) {
