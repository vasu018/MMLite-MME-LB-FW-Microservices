...
#endif
                        kvs_set_64(IMSI_HTABLE, (s1ap_packet->message.ue_init_msg.value.
...
int create_dummy_hss_auth_req(struct rte_mbuf* packet,uint32_t enb_ue_s1ap_id,pkt_identifier_t *pkt_identifier)
{
    ...
    //ipv4_hdr->hdr_checksum = ip_sum((unaligned_uint16_t *)ipv4_hdr,sizeof(*ipv4_hdr));
	
	
	ip_options_type * ip_opt=(ip_options_type *)rte_pktmbuf_append(packet, sizeof(ip_options_type));
	memcpy(&ip_opt->ip_options[0], &enb_ue_s1ap_id, sizeof(uint32_t));
	
	struct udp_hdr *udp_h=(struct udp_hdr *)rte_pktmbuf_append(packet, sizeof(struct udp_hdr));
	memset(udp_h,0,sizeof(struct udp_hdr));
	...
	printf("headers memset to 0\n\n");
	
	char * udp_data;
	udp_data= (char *)rte_pktmbuf_append(packet, TXONLY_DEF_PACKET_LEN -2 -sizeof(pkt_identifier_t) -sizeof(struct ether_hdr) -sizeof(struct ipv4_hdr));


    memcpy(udp_data, pkt_identifier, sizeof(pkt_identifier_t));
	printf("packet identifier copied\n");
	memcpy(udp_data + sizeof(pkt_identifier_t), payload,(TXONLY_DEF_PACKET_LEN - 2) -sizeof(struct ether_hdr) -sizeof(struct ipv4_hdr));
	printf("packet payload copied\n");
	packet->nb_segs             = 1;
...
int bypass_hss_send_auth_req(struct rte_mbuf* pkt)
{
		...
		if((ip->version_ihl&0x0f) > 5) 
                ip_options_length = ((ip->version_ihl&0x0f) - 5) * 32;
	#ifdef LOGS
		printf("Bypassing HSS:ip_options_length %d \n",ip_options_length);
	#endif
		pkt_identifier_t *pkt_identifier =  (pkt_identifier_t*)(rte_pktmbuf_mtod(pkt, uint8_t*) + sizeof(struct ether_hdr) + sizeof(struct ipv4_hdr) + ip_options_length + sizeof(struct udphdr));
		
		struct rte_mbuf *auth_req;
		auth_req = rte_pktmbuf_alloc(pktmbuf_pool);
		//auth_req = (struct rte_mbuf *)malloc(sizeof(struct rte_mbuf));
		s1ap_packet_type s1ap_packet;
        s1ap_packet_parser(pkt, &s1ap_packet);
		uint32_t enb_ue_s1ap_id = get_enb_ue_s1ap_id(&s1ap_packet);
	#ifdef LOGS
		printf("Bypassing HSS:enb_ue_s1ap_id %d \n",enb_ue_s1ap_id);
	#endif
		create_dummy_hss_auth_req(auth_req,enb_ue_s1ap_id,pkt_identifier);
        
		s6a_packet_type s6a_packet;
		s6a_packet_parser(auth_req,  &s6a_packet);
	...
	#ifdef LOGS
		RTE_LOG(INFO, APP, "Bypassing HSS to send auth req to UE\n");
		printf("Bypassing HSS:enb_ue_s1ap_id %d \n",enb_ue_s1ap_id);
	#endif
...
s1ap_mme_handle_initial_ue_message(struct rte_mbuf* pkt, uint8_t slice_id) 
{
	...
	if(get_msg_type(s1ap_packet) == DETACH_REQUEST) {
#ifdef LOGS
		RTE_LOG(INFO, APP, "\n\n\n");
		RTE_LOG(INFO, APP, "*************** Received Detatch Request ****************\n");
#endif
		if(ue_context != NULL) {
			//TODO save NAS security context into ue context
			msg_type = PKT_TYPE_DETACH;
			// Update udp port number. We should save detach
			// port number differently. But honestly we should be doing
			// a lot of things which we aren't ¯\_(ツ)_/¯
...
#endif		
			ue_context->conn_state = UE_STATE_DETATCH_DELETE_SESSION;
			nas_send_detatch_accpet(get_enb_ue_s1ap_id(s1ap_packet));
			nas_send_ue_context_release_req(get_enb_ue_s1ap_id(s1ap_packet));
			free(s1ap_packet);
			return 0;
#endif
			s11_send_delete_session_request(get_enb_ue_s1ap_id(s1ap_packet), msg_type, slice_id);
			free(s1ap_packet);
			msg_type = PKT_TYPE_DETACH;
			return 0;
		}
	...
	} else if(get_msg_type(s1ap_packet) == SERVICE_REQUEST) {
#ifdef LOGS
		RTE_LOG(INFO, APP, "\n\n\n");
...
#ifdef BYPASS_HSS
            RTE_LOG(INFO, APP, "*************** Bypassing HSS ******************\n");
			bypass_hss_send_auth_req(pkt);
			return 0;
#endif
		} else {
			RTE_LOG(INFO, APP, "ERROR:  UE Context not found \n");
...
#endif
		//if(kvs_get_64(IMSI_HTABLE,(get_imsi(s1ap_packet))) == NULL) {
			...
			ue_context->slice_id = slice_id;
			ue_context->e_utran_cgi = 0x01;  //dummy value
#ifdef TIME_STATS
			ue_context->time_stamp[INDEX_ATTACH_REQ_IN] = in_time;
#endif
			/*
			...
			ue_context->udp_port = ntohs(udp->src_port);
			//              RTE_LOG(INFO, APP, "get_enb_ue_s1ap_id = 0x%x \n", get_enb_ue_s1ap_id(s1ap_packet));
			kvs_set_64(IMSI_HTABLE, (get_imsi(s1ap_packet)), (void *)ue_context);
...
s1ap_mme_handle_uplink_nas_transport(struct rte_mbuf* pkt, uint8_t msg_type, uint8_t slice_id)
{
...
#ifdef LOGS
	RTE_LOG(INFO, APP, "\nMessage type:0x%x\n",s1ap_packet.message.ue_init_msg.value.init_msg.pdus[i].pdu.nas_pdu.MM_message_type);
#endif
	...
	#ifdef BYPASS_HSS
			RTE_LOG(INFO, APP, "\n\n\n");
	        RTE_LOG(INFO, APP, "*************** Bypassing HSS and SGW and sending attach accept to UE****************\n");
			nas_itti_establish_cnf(get_enb_ue_s1ap_id(&s1ap_packet));
			return 0;
	#endif
		}
...
//                printf("\n%s:%d\n",__func__,get_enb_ue_s1ap_id(&s1ap_packet));
                s1ap_handle_attach_complete_message(get_enb_ue_s1ap_id(&s1ap_packet), msg_type, slice_id);
        }
...
s1ap_handle_attach_complete_message(uint32_t enb_ue_s1ap_id, uint8_t msg_type, uint8_t slice_id){
	...
	ue_context->conn_state = UE_STATE_ATTACH_COMPLETE;

#ifdef TIME_STATS
	ue_context->time_stamp[INDEX_ATTACH_COMPLETE_IN] = in_time;
#ifdef LOGS
	printf("Attach complete in time = %lf", ue_context->time_stamp[INDEX_ATTACH_COMPLETE_IN]);
#endif
#endif


	s11_send_modify_bearer_req(enb_ue_s1ap_id, msg_type, slice_id);
	return 0;
}
