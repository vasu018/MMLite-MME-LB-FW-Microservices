...
typedef enum {
    ...
    UE_STATE_TAU_REQ,
    UE_STATE_DETATCH_REQ,
    UE_STATE_DETATCH_DELETE_SESSION
}conn_state_type;
...
//  ambr_t                 used_ambr;
#else
   uint8_t		apn_profile[30];                  // set by S6A UPDATE LOCATION ANSWER
   uint32_t                  access_restriction_data;      // set by S6A UPDATE LOCATION ANSWER
   uint32_t    sub_status;                   // set by S6A UPDATE LOCATION ANSWER
   uint32_t                 subscribed_ambr;              // set by S6A UPDATE LOCATION ANSWER
   uint32_t                 used_ambr;
#endif
