...
//			imsi_g[j] = nas_pdu->MM_message.attach_req.EPS_id.identity[j];
	...
	case 0x43:
        ...
        default:
                RTE_LOG(INFO, APP, "Unknown mobility message\n");
                break;
	}
...
int nas_itti_pdn_connectivity_req(uint8_t * imsi, uint32_t enb_ue_s1ap_id){
        ...
        pdn_req_query = malloc(100);
        strcpy(pdn_req_query, "SELECT * FROM `pdn` WHERE `pdn`.`users_imsi`=");
        strcat(pdn_req_query, (char*)imsi);
        strcat(pdn_req_query, " LIMIT 10;");
        pdn_req = rte_pktmbuf_alloc(pktmbuf_pool);
        pmeta = onvm_get_pkt_meta(pdn_req);
        pmeta->destination = 2;
        pmeta->action = ONVM_NF_ACTION_TONF;
        //pmeta->flags = ONVM_SET_BIT(0, SPEED_TESTER_BIT);
        identifier = MME_LOC_UPDATE_GET_PDN;
        memcpy(((char*)pdn_req->buf_addr + pdn_req->data_off), &identifier, sizeof(uint8_t));
	memcpy(((char*)pdn_req->buf_addr + pdn_req->data_off+ sizeof(uint8_t)), &enb_ue_s1ap_id, sizeof(uint32_t));
        memcpy(((char*)pdn_req->buf_addr + pdn_req->data_off + sizeof(uint8_t) + sizeof(uint32_t)), pdn_req_query, strlen(pdn_req_query)+1);
        //auth_req_query->hash.rss = i;
        onvm_nflib_return_pkt(pdn_req);
//      printf("imsi = %s", imsi);
        printf("pdn_req_query = %s", ((char*)pdn_req->buf_addr + pdn_req->data_off));
        return 0;
}
...
int nas_send_mmeinfo_req(uint8_t * imsi, uint32_t enb_ue_s1ap_id){
        struct rte_mbuf* mmeinfo_req1;
        struct rte_mbuf* mmeinfo_req2;
        struct onvm_pkt_meta* pmeta1;
        struct onvm_pkt_meta* pmeta2;
        char* mmeinfo_req_query1;
        char* mmeinfo_req_query2;

        struct rte_mempool *pktmbuf_pool1;
        struct rte_mempool *pktmbuf_pool2;
        int i;
	char idmmeidentity = IDMMEIDENTITY + '0';
        uint8_t identifier;

        pktmbuf_pool1 = rte_mempool_lookup(PKTMBUF_POOL_NAME);
        pktmbuf_pool2 = rte_mempool_lookup(PKTMBUF_POOL_NAME);
        if(pktmbuf_pool1 == NULL || pktmbuf_pool2 == NULL) {
                onvm_nflib_stop();
                rte_exit(EXIT_FAILURE, "Cannot find mbuf pool!\n");
        }
        ...
        mmeinfo_req_query1 = malloc(100);
        mmeinfo_req_query2 = malloc(100);
        strcpy(mmeinfo_req_query1, "SELECT `access_restriction`,`mmeidentity_idmmeidentity`,`msisdn`,`ue_ambr_ul`,`ue_ambr_dl`,`rau_tau_timer` FROM `users` WHERE `users`.`imsi`='");
        strcat(mmeinfo_req_query1, (char*)imsi);
        strcat(mmeinfo_req_query1, "'");
        mmeinfo_req1 = rte_pktmbuf_alloc(pktmbuf_pool1);
        pmeta1 = onvm_get_pkt_meta(mmeinfo_req1);
        pmeta1->destination = 2;
        pmeta1->action = ONVM_NF_ACTION_TONF;
        //pmeta->flags = ONVM_SET_BIT(0, SPEED_TESTER_BIT);
        identifier = MME_LOC_UPDATE_GET_USR_DATA;
        memcpy(((char*)mmeinfo_req1->buf_addr + mmeinfo_req1->data_off), &identifier, sizeof(uint8_t));
	memcpy(((char*)mmeinfo_req1->buf_addr + mmeinfo_req1->data_off+ sizeof(uint8_t)), &enb_ue_s1ap_id, sizeof(uint32_t));
        memcpy(((char*)mmeinfo_req1->buf_addr + mmeinfo_req1->data_off + sizeof(uint8_t) + sizeof(uint32_t)), mmeinfo_req_query1, strlen(mmeinfo_req_query1)+1);
        //auth_req_query->hash.rss = i;
        onvm_nflib_return_pkt(mmeinfo_req1);
//      printf("imsi = %s", imsi);
        printf("mmeinfo_req_query = %s", ((char*)mmeinfo_req1->buf_addr + mmeinfo_req1->data_off));


        strcpy(mmeinfo_req_query2, "SELECT mmehost,mmerealm FROM mmeidentity WHERE mmeidentity.idmmeidentity='");
        strcat(mmeinfo_req_query2, &idmmeidentity);
        strcat(mmeinfo_req_query2, "'");
        mmeinfo_req2 = rte_pktmbuf_alloc(pktmbuf_pool2);
        pmeta2 = onvm_get_pkt_meta(mmeinfo_req2);
        pmeta2->destination = 2;
        pmeta2->action = ONVM_NF_ACTION_TONF;
        //pmeta->flags = ONVM_SET_BIT(0, SPEED_TESTER_BIT);
        identifier = MME_LOC_UPDATE_GET_MMEID;
        memcpy(((char*)mmeinfo_req2->buf_addr + mmeinfo_req2->data_off), &identifier, sizeof(uint8_t));
	memcpy(((char*)mmeinfo_req2->buf_addr + mmeinfo_req2->data_off+ sizeof(uint8_t)), &enb_ue_s1ap_id, sizeof(uint32_t));
        memcpy(((char*)mmeinfo_req2->buf_addr + mmeinfo_req2->data_off + sizeof(uint8_t) + sizeof(uint32_t)), mmeinfo_req_query2, strlen(mmeinfo_req_query2)+1);
        //auth_req_query->hash.rss = i;
        onvm_nflib_return_pkt(mmeinfo_req2);
//      printf("imsi = %s", imsi);
        printf("mmeinfo_req_query = %s", ((char*)mmeinfo_req2->buf_addr + mmeinfo_req2->data_off));
        return 0;
}
...
nas_itti_establish_cnf_debug(struct rte_mbuf* pkt, uint32_t enb_ue_s1ap_id)
{
        ...
        uint8_t packet[] = { 0x02, 0x0b, 0x00, 0x23, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x08, 0x00, 0x04, 0x80, 0x64, 0x6d, 0xbe, 0x00, 0x1a, 0x00, 0x0e, 0x0d, 0x37, 0x5f, 0x28, 0x8b, 0x28, 0x00, 0x07, 0x5d, 0x02, 0x00, 0x02, 0x80, 0x20, 0x00};
//	uint32_t enb_ue_s1ap_id;
	uint32_t * enb_ue_s1ap_id_p = (uint32_t *)(&packet[0] + 17); //17
        ue_context_t * ue_context;
#if 0
	ip_options_type * ip_opt;
	...
	ip_opt= mme_pkt_ipopt_hdr(pkt);
        ...
        pmeta->destination = 0;
        pmeta->action = ONVM_NF_ACTION_OUT;
        pkt_identifier_type pkt_id;
        pkt_id.hostid = nf_info->service_id;
        /*this is a hack there should have been a field called message_type*/
        pkt_id.cluster_id =0x04;
	memcpy(enb_ue_s1ap_id_p, &enb_ue_s1ap_id, sizeof(uint32_t));
	memcpy(buf, &pkt_id, sizeof(pkt_identifier_type));
	memcpy(buf + sizeof(pkt_identifier_type), packet, sizeof(packet));

	printf("enb_ue_s1ap_id = %x", enb_ue_s1ap_id);
        ue_context = kvs_get(ENB_HTABLE, enb_ue_s1ap_id);

	frame_packet(seq_command, &buf[0], ue_context);
        RTE_LOG(INFO, APP, "\n\n\n");
        RTE_LOG(INFO, APP, "*************** ATTACH ACCEPT sent to UE ****************\n");
        onvm_nflib_return_pkt(seq_command);
        return 0;
